 ...
 *
 * @constructor
 * @param {MWParserEnvironment} conf
 * @param {Boolean} inTemplate
 */
class TokenSanitizer {
	constructor( conf, inTemplate ) {
		this.conf = conf;
	...
	sanitizeTagAttrs( token, attrs ) {
		...
		for ( let i = 0; i < n; i++ ) {
			...
			// that instead.
				...
				let newHref = TokenSanitizer.cleanUrl( this.conf, v, mode );
				if ( newHref !== v ) {
