...
const semver = require( 'semver' ),
	JSUtils = require( './utils/jsutils' ).JSUtils,
	Util = require( './utils/Util' ).Util,
	defines = require( './parser.defines' ),
	WikitextConstants = require( './config/WikitextConstants' ).WikitextConstants;

// define some constructor shortcuts
let TagTk = defines.TagTk;
let SelfclosingTagTk = defines.SelfclosingTagTk;
let EndTagTk = defines.EndTagTk;
...
Sanitizer.stripIDNs = function ( host ) {
...
Sanitizer.cleanUrl = function ( env, href, mode ) {
	...
	if ( bits ) {
		...
		host = Sanitizer.stripIDNs( bits[ 2 ] );
...
Sanitizer.prototype.sanitizeToken = function ( token ) {
	...
	let noEndTagSet = this.constants.noEndTagSet;
...
Sanitizer.prototype.sanitizeTagAttrs = function ( token, attrs ) {
	let tag = token.name;
	let allowRdfa = this.constants.globalConfig.allowRdfaAttrs;
	let allowMda = this.constants.globalConfig.allowMicrodataAttrs;
	let html5Mode = this.constants.globalConfig.html5Mode;
	let xmlnsRE = this.constants.XMLNS_ATTRIBUTE_RE;
	let evilUriRE = this.constants.EVIL_URI_RE;
	...
	for ( let i = 0; i < n; i++ ) {
		...
		if ( k === 'href' || k === 'src' || k === 'poster' ) { // T163583
			...
			let newHref = Sanitizer.cleanUrl( this.env, v, mode );
			if ( newHref !== v ) {
